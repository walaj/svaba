...
void runSplitCounter(int argc, char** argv) {
  ...
  while(walk.GetNextRead(r, rule)) {
    ...
    if (cig[0].Type() == 'H' || cig[0].Type() == 'S')  // split on left
      std::cout << "L\t" << r.ChrName(walk.header()) << "\t" << r.Position() << "\t" << (r.ReverseFlag() ? "-" : "+") << "\t" << r.Qname() << "\t" << r.MapQuality() << "\t" << (r.SecondaryFlag() ? "SEC" : "PRI") << "\t" << (bks.findOverlapping(r.asGenomicRegion()) ? "IN_BK" : "NO_BK") << std::endl;

    if (cig.back().Type() == 'H' || cig.back().Type() == 'S')  // split on left
      std::cout << "R\t" << r.ChrName(walk.header()) << "\t" << r.PositionEnd() << "\t" << (r.ReverseFlag() ? "-" : "+") << "\t" << r.Qname() << "\t" << r.MapQuality() << "\t" << (r.SecondaryFlag() ? "SEC" : "PRI") << "\t" << (bks.findOverlapping(r.asGenomicRegion()) ? "IN_BK" : "NO_BK") << std::endl;
